...
public class AuthController {
    ...
    public AuthController(AuthService service, UserServices userServices) {
    ...
    @PostMapping("/login")
    public ResponseEntity<ApiResponseDto<?>> signIn(@RequestBody LoginRequestDto user, HttpServletResponse response) {
        try {
            log.info("User login request received for email: {}", user.getEmail());

            if (!user.isValid() || !user.isEmailValid()) {
                return ResponseEntity.badRequest()
                        .body(ApiResponseDto.error("Invalid email or password format."));
            }

            // The service will now handle authentication and return the user if successful
            Optional<User> authenticatedUserOptional = service.validateUser(user, response);
            ...
            if (authenticatedUserOptional.isPresent()) {
                ...
                return ResponseEntity.ok(ApiResponseDto.success("Login successful", authenticatedUserOptional.get()));
            } else {
                // This case is technically handled by the exception below, but it's good practice
            ...
            log.warn("Invalid credentials for email: {}", user.getEmail());
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(ApiResponseDto.error("Invalid email or password"));
        } catch (Exception e) {
            log.error("Login error: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponseDto.error("An internal server error occurred."));
        }
    }
